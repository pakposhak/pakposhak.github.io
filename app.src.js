  ...
  const SUPPORT_WA       = '8801775288301';
  // ═══════════════════════════════════════════════════════════════════════
  ...
  const I18N = {
    en: {
      ...
      hiw_pay:'Pay via bKash/Nagad', hiw_delivery:'2–3 weeks to your door', hiw_video:'▶ Watch video guide', hiw_guide:'📖 How it works',
      ...
      bb_more:'more', bb_less:'less', bb_all:'All', bb_two_ways:'Two ways to search brands', bb_pick_gender:'Pick women, men, or others above — or just type any brand name.', bb_pick_cat:'Pick a category above to see its brands.', bb_prod_sub:'Choose your brand by clicking a tab or typing its name, go to the product you like, and just share it back to us.', bb_prod_sub_check:'Search by brand name or what it sells — tap a brand to check its price.', bb_checkprice_m:"To check a price and order, share the product's\nlink with us, or copy and paste it here.\nYou'll see the total landed price in BDT.", bb_checkvid:'Unsure? Check video', sb_ph_brands:'Search brands only', bb_loading:'Loading brands…', bb_prod_none:'No catalogued brands here yet.',
    ...
    bn: {
      ...
      hiw_pay:'বিকাশ/নগদে পেমেন্ট', hiw_delivery:'২–৩ সপ্তাহে আপনার দরজায়', hiw_video:'▶ ভিডিও গাইড দেখুন', hiw_guide:'📖 কীভাবে কাজ করে',
  ...
  function primePaymentConfirm(totalBdt){
    ...
    const totBar = document.getElementById('payTotalAmt');
    if(totBar) totBar.textContent = totalBdt ? ('৳' + Number(totalBdt).toLocaleString()) : '৳0';
    window.__payOverride = false;
  }
  ...
  // Copy JUST the digits of the order total, so the buyer can paste it straight into the
  // bKash / Nagad / bank amount field (no ৳ or commas). Reuses copyNum's clipboard + feedback.
  function copyPayAmount(btn){
    const box = document.getElementById('payConfirmBox');
    const raw = String((box && box.dataset.total) || '').replace(/[^0-9]/g, '');
    if(raw) copyNum(raw, btn);
  }
  window.copyPayAmount = copyPayAmount;
  ...
  function psAddedToBagToast(){
    ...
    t.classList.add('on');
    // COLLISION FIX (regression): this toast shares the bottom-right with the Paste FAB + swipe cue.
    // Mirror psShareAddedToast — suppress them while it's up, restore when it dismisses. updatePasteFab
    // also checks psShareToastActive, so it can't re-show the FAB underneath the toast meanwhile.
    psShareToastActive = true;
    var _pf = document.getElementById('pasteFab'); if(_pf) _pf.classList.remove('show','pulse');
    var _sc = document.getElementById('psSwipeCue'); if(_sc) _sc.classList.remove('show');
    clearTimeout(t._h); t._h = setTimeout(function(){ if(t) t.classList.remove('on'); psShareToastActive = false; try{ updatePasteFab(); }catch(e){} }, 3800);
  }
  ...
  var _psVisBrands = new Set();   // empty = all brands
  var _psVisStitch = 'pret';      // 'pret' | 'unstitch' | 'all'
  // Visual-search brand filter: a themed multi-select dropdown with typeahead. Master list is the
  ...
  function _psVisBrandLabel(){
    if(_psVisBrands.size===0) return tr('vis_allbrands');
    if(_psVisBrands.size===1) return Array.from(_psVisBrands)[0];
    return _psVisBrands.size+' '+tr('vis_brands_n');
  }
  function psVisBrandDDBuild(){
    var wrap=document.getElementById('psVisBrandDD'); if(!wrap) return;
    var all=_psVisBrandsAll();
    var lis='<li data-v="__all" role="option"'+(_psVisBrands.size===0?' class="sel"':'')+'>'+esc(tr('vis_allbrands'))+'</li>'
      + all.map(function(b){ return '<li data-v="'+esc(b)+'" role="option"'+(_psVisBrands.has(b)?' class="sel"':'')+'>'+esc(b)+'</li>'; }).join('');
    wrap.innerHTML='<button type="button" class="ps-dd-btn"><span>'+esc(_psVisBrandLabel())+'</span></button>'
      +'<div class="ps-dd-list" hidden role="listbox"><input type="text" class="ps-dd-search" placeholder="'+esc(tr('dd_search'))+'" aria-label="'+esc(tr('dd_search'))+'"><ul class="ps-dd-ul">'+lis+'</ul></div>';
    if(!wrap._wired){ wrap._wired=true;
      wrap.addEventListener('click',function(e){
        var li=e.target.closest('li[data-v]');
        if(li){ var v=li.getAttribute('data-v'); if(v==='__all'){ _psVisBrands.clear(); } else if(_psVisBrands.has(v)){ _psVisBrands.delete(v); } else { _psVisBrands.add(v); } psVisBrandDDSync(); return; }
        if(e.target.closest('.ps-dd-search')) return;            // typing in search must not toggle the dropdown
        if(e.target.closest('.ps-dd-btn')) psDDToggle('psVisBrand');
      });
      wrap.addEventListener('input',function(e){ if(e.target.classList.contains('ps-dd-search')) psDDFilter(wrap, e.target.value); });
    }
  }
  function psVisBrandDDSync(){           // multi-select: refresh button label + ticks, keep the list OPEN
    var wrap=document.getElementById('psVisBrandDD'); if(!wrap) return;
    var span=wrap.querySelector('.ps-dd-btn>span'); if(span) span.textContent=_psVisBrandLabel();
    [].forEach.call(wrap.querySelectorAll('li[data-v]'),function(li){ var v=li.getAttribute('data-v'); li.classList.toggle('sel', v==='__all'?_psVisBrands.size===0:_psVisBrands.has(v)); });
  }
  function psVisSetStitch(btn,val){
    _psVisStitch=val;
    var seg=document.getElementById('psVisStitchSeg'); if(!seg) return;
    seg.querySelectorAll('button').forEach(function(b){ b.classList.toggle('on', b.dataset.s===val); });
  }
  function psVisualSearch(){ psVisualSheet(true); }
  function psVisualSheet(show){ var s=document.getElementById('psVisSheet'); if(s){ s.hidden=!show; document.body.style.overflow = show ? 'hidden' : ''; if(show) psVisBrandDDBuild(); } }
  ...
  function psVisualRun(dataUrl){
    ...
    fetch(psSearchBase()+'/visual',{ method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({image:dataUrl,k:48}), signal:_vac.signal })
      ...
      .then(function(j){
        ...
        if(!items.length){ psVisualToast(notCarried>=3?tr('vis_notcarried'):tr('vis_none')); return; }
        psVisualShow(items);
      })
  ...
  function psVisualShow(items){
    ...
    try{
      var _vc=document.getElementById('psVisChip')||document.getElementById('psGrid');
      if(_vc){ window.scrollTo({top:Math.max(0,_vc.getBoundingClientRect().top+window.pageYOffset-70),behavior:'smooth'}); }
    }catch(e){}
  }
  // shared "frozen results" chip — used by BOTH photo search and Fit Assistant (kind sets icon/colour)
  function psVisualChip(show, text, kind, sub){
    ...
    if(show){
      ...
      var body = sub ? '<span><strong>'+esc(text)+'</strong><small>'+esc(sub)+'</small></span>' : '<span>'+esc(text)+'</span>';
      chip.innerHTML=icon+body+'<button type="button" onclick="psVisualClear()" aria-label="'+esc(tr('vis_clear'))+'">✕</button>';
      chip.hidden=false;
  ...
  var PS_FIT_SIZES_SHIRT=["14","14.5","15","15.5","16","16.5","17","17.5"];
  var PS_FIT_SIZES_BOTTOM_W=["24","26","28","30","32","34","36","38","40","42"];
  var PS_FIT_SIZES_BOTTOM_M=["28","30","32","34","36","38","40","42","44","46"];
  // Men's dress-shirt collar (neck, inches) -> approximate CHEST (inches). Collar/neck is not a
  // dimension the fit engine models, so the Shirts tab matches on CHEST instead; the collar pick
  // just seeds an EDITABLE chest estimate the buyer can correct. chest = (collar-15)*4 + 38.
  var PS_COLLAR_CHEST={"14":34,"14.5":36,"15":38,"15.5":40,"16":42,"16.5":44,"17":46,"17.5":48};
  function psCollarToChest(c){ if(PS_COLLAR_CHEST[c]!=null) return PS_COLLAR_CHEST[c]; var n=parseFloat(c); return n>0?Math.round((n-15)*4+38):''; }
  ...
  function psFitTypeSet(t){
    psFitType=t;
    var seg=document.getElementById('psFitTypeSeg'); if(seg) [].forEach.call(seg.children,function(b){ b.classList.toggle('on', b.getAttribute('data-t')===t); });
    var row=document.getElementById('psFitBrandRow');
    var bd=row&&row.querySelector('.ps-dd[data-dd="psFitBrand"]');
    var bm=row&&row.querySelector('.ps-fit-mid');
    var sd=row&&row.querySelector('.ps-dd[data-dd="psFitSize"]');
    var isBrand=(t==='top');
    if(bd) bd.hidden=!isBrand;
    if(bm) bm.hidden=!isBrand;
    if(sd){ sd.classList.toggle('ps-fit-dd-full',!isBrand); }
    var sr=document.getElementById('psFitShirtRow'); if(sr) sr.hidden=(t!=='shirt');
    var lbl=document.getElementById('psFitBlockLbl'); if(lbl) lbl.textContent=tr(t==='shirt'?'fit_wear_collar':t==='bottom'?'fit_wear_waist':'fit_wear');
    psFitFillSizes();
    if(t==='shirt') psFitSeedChest();
  }
  // seed the editable chest field on the Shirts tab from the chosen collar. force=true overwrites
  // (collar changed); force=false only fills when empty (first open) so a typed chest isn't lost.
  function psFitSeedChest(force){
    var ci=document.getElementById('psFitShirtChest'); if(!ci) return;
    if(!force && ci.value) return;
    var est=psCollarToChest((document.getElementById('psFitSize')||{}).value); if(est) ci.value=est;
  }
  // ── themed dropdown (.ps-dd) — custom, theme-matched replacement for native <select> ──
  // Each .ps-dd[data-dd="<id>"] mirrors a hidden <select id="<id>">; the select holds the
  // value (read by psFitRun), the .ps-dd renders the themed button + option list.
  function psDDBuild(key){
    var sel=document.getElementById(key); var wrap=document.querySelector('.ps-dd[data-dd="'+key+'"]');
    if(!sel||!wrap) return;
    var cur=sel.value, curOpt=sel.options[sel.selectedIndex];
    var lis=[].map.call(sel.options,function(o){ return '<li role="option" data-v="'+esc(o.value)+'"'+(o.value===cur?' class="sel"':'')+'>'+esc(o.textContent)+'</li>'; }).join('');
    // Long lists (e.g. the 45-brand Fit reference) get a typeahead search box; short ones don't need it.
    var srch = sel.options.length>8 ? '<input type="text" class="ps-dd-search" placeholder="'+esc(tr('dd_search'))+'" aria-label="'+esc(tr('dd_search'))+'">' : '';
    wrap.innerHTML='<button type="button" class="ps-dd-btn"><span>'+esc(curOpt?curOpt.textContent:'')+'</span></button><div class="ps-dd-list" hidden role="listbox">'+srch+'<ul class="ps-dd-ul">'+lis+'</ul></div>';
    if(!wrap._wired){ wrap._wired=true;
      wrap.addEventListener('click',function(e){
        var li=e.target.closest('li[data-v]');
        if(li){ psDDChoose(key, li.getAttribute('data-v')); return; }
        if(e.target.closest('.ps-dd-search')) return;            // typing in search must not toggle the dropdown
        if(e.target.closest('.ps-dd-btn')) psDDToggle(key);
      });
      wrap.addEventListener('input',function(e){ if(e.target.classList.contains('ps-dd-search')) psDDFilter(wrap, e.target.value); });
    }
  }
  // Typeahead: hide option <li>s that don't match the typed query; show a "no match" row if none do.
  function psDDFilter(wrap, q){
    q=(q||'').trim().toLowerCase();
    var ul=wrap.querySelector('.ps-dd-ul'); if(!ul) return;
    var any=false;
    [].forEach.call(ul.querySelectorAll('li[data-v]'),function(li){
      if(li.getAttribute('data-v')==='__all'){ li.style.display=q?'none':''; return; }   // hide "All" while searching
      var hit=!q||li.textContent.toLowerCase().indexOf(q)>=0; li.style.display=hit?'':'none'; if(hit) any=true;
    });
    var empty=ul.querySelector('.ps-dd-empty');
    if(!any){ if(!empty){ empty=document.createElement('li'); empty.className='ps-dd-empty'; ul.appendChild(empty); } empty.textContent=tr('dd_none'); empty.style.display=''; }
    else if(empty){ empty.style.display='none'; }
  }
  function psDDToggle(key){
    var wrap=document.querySelector('.ps-dd[data-dd="'+key+'"]'); if(!wrap) return;
    var willOpen=!wrap.classList.contains('open'); psDDCloseAll();
    if(willOpen){ wrap.classList.add('open'); wrap.classList.remove('up'); var l=wrap.querySelector('.ps-dd-list'); if(l){ l.hidden=false;
      var srch=l.querySelector('.ps-dd-search'); if(srch){ srch.value=''; psDDFilter(wrap,''); setTimeout(function(){ try{ srch.focus(); }catch(e){} },30); }
      var s=l.querySelector('li.sel'); if(s) s.scrollIntoView({block:'nearest'});
      // In a bottom-sheet the trigger sits low, so a long list would spill below the fold. If it
      // overflows the viewport bottom and there is more room above the button, flip it open upward.
      try{ var lr=l.getBoundingClientRect(), br=wrap.querySelector('.ps-dd-btn').getBoundingClientRect();
        if(lr.bottom > window.innerHeight-8 && br.top > (window.innerHeight - br.bottom)) wrap.classList.add('up'); }catch(e){} } }
  }
  function psDDCloseAll(){ [].forEach.call(document.querySelectorAll('.ps-dd.open'),function(w){ w.classList.remove('open'); var l=w.querySelector('.ps-dd-list'); if(l) l.hidden=true; }); }
  function psDDChoose(key,val){
    var sel=document.getElementById(key); if(sel) sel.value=val;
    psDDBuild(key); psDDCloseAll();
    if(key==='psFitBrand') psFitFillSizes();   // picking a brand refreshes its real sizes
    if(key==='psFitSize' && psFitType==='shirt') psFitSeedChest(true);   // collar -> seed chest
  }
  document.addEventListener('click',function(e){ if(!e.target.closest('.ps-dd')) psDDCloseAll(); });
  function psFitFillBrands(){
    var list=PS_FIT_BRANDS[psFitG]||[];
    var rb=document.getElementById('psFitBrand'); if(rb){ var cur=rb.value; rb.innerHTML=list.map(function(b){return '<option>'+esc(b)+'</option>';}).join(''); if(list.indexOf(cur)>=0) rb.value=cur; psDDBuild('psFitBrand'); }
    var sc=document.getElementById('psFitScope'); if(sc){ var cs=sc.value; sc.innerHTML='<option value="">'+esc(tr('fit_allbrands'))+'</option>'+list.map(function(b){return '<option>'+esc(b)+'</option>';}).join(''); if(cs) sc.value=cs; psDDBuild('psFitScope'); }
    psFitFillSizes();
  }
  // sizes depend on type: top = brand chart, shirt = collar inches, bottom = waist inches
  function psFitFillSizes(){
    var sizes;
    if(psFitType==='shirt') sizes=PS_FIT_SIZES_SHIRT;
    else if(psFitType==='bottom') sizes=(psFitG==='w'?PS_FIT_SIZES_BOTTOM_W:PS_FIT_SIZES_BOTTOM_M);
    else{ var brand=(document.getElementById('psFitBrand')||{}).value; sizes=(PS_FIT_SIZES[psFitG]&&PS_FIT_SIZES[psFitG][brand])||PS_FIT_SIZES_GENERIC; }
    var sz=document.getElementById('psFitSize'); if(!sz) return;
    var cur=sz.value;
    sz.innerHTML=sizes.map(function(s){return '<option>'+esc(s)+'</option>';}).join('');
    if(sizes.indexOf(cur)>=0) sz.value=cur; else sz.selectedIndex=Math.min(Math.floor(sizes.length/2), sizes.length-1);
    psDDBuild('psFitSize');
  }
  function psFitRun(){
    var g=psFitG, params='fgender='+g+'&ftype='+psFitType, basis;
    if(psFitMeasOpen){
      var ln=parseFloat((document.getElementById('psFitLength')||{}).value), ch=parseFloat((document.getElementById('psFitChest')||{}).value), wa=parseFloat((document.getElementById('psFitWaist')||{}).value), hi=parseFloat((document.getElementById('psFitHip')||{}).value);
      if(!(ch>0)&&!(ln>0)){ psVisualToast(tr('fit_needchest')); return; }
      params+=(ln>0?'&flength='+ln:'')+(ch>0?'&fchest='+ch:'')+(wa>0?'&fwaist='+wa:'')+(hi>0?'&fhip='+hi:'')+'&ffit='+psFitFitPref;
      basis=(ch>0?tr('fit_chest')+' '+ch+'"':'')+(ln>0?(ch>0?' · ':'')+tr('fit_length').replace(/\s*\(.*\)/,'')+' '+ln+'"':'');
    } else {
      var sz=(document.getElementById('psFitSize')||{}).value;
      if(!sz){ psVisualToast(tr('fit_needref')); return; }
      if(psFitType==='top'){
        var br=(document.getElementById('psFitBrand')||{}).value;
        if(!br){ psVisualToast(tr('fit_needref')); return; }
        params+='&fref='+encodeURIComponent(br)+'&fsize='+encodeURIComponent(sz);
        basis=br+' · '+sz;
      } else if(psFitType==='bottom'){
        // waist (inches) IS a body measurement the engine accepts — route through the measurement path
        params+='&fwaist='+encodeURIComponent(sz)+'&ffit='+psFitFitPref;
        basis=tr('fit_chip_waist')+' '+sz+'"';
      } else {   // shirt: match on CHEST (collar can't be translated); chest seeded from collar, editable
        var chest=parseFloat((document.getElementById('psFitShirtChest')||{}).value)||psCollarToChest(sz);
        if(!(chest>0)){ psVisualToast(tr('fit_needchest')); return; }
        params+='&fchest='+encodeURIComponent(chest)+'&ffit='+psFitFitPref;
        basis=tr('fit_chip_collar')+' '+sz+'" · '+tr('fit_chest')+' '+chest+'"';
      }
    }
  ...
  window.psVisualSheet=psVisualSheet; window.psVisualPick=psVisualPick; window.psVisualFile=psVisualFile; window.psVisualClear=psVisualClear; window.psVisSetStitch=psVisSetStitch;
  window.psFitSheet=psFitSheet; window.psFitGender=psFitGender; window.psFitFitSet=psFitFitSet; window.psFitToggleMeas=psFitToggleMeas; window.psFitTypeSet=psFitTypeSet; window.psFitRun=psFitRun; window.psLoadCancel=psLoadCancel; window.psVimgCancel=psVimgCancel;
  ...
  function switchBrowse(which){
    ...
    document.body.classList.toggle('psb-brands', brands);   // Price-Check (brands) page: mobile top-search label swaps to "Search brands only" (CSS)
  ...
  const PS_POSTERS = {
    'home-all': [
      ...
      // Change the background, keeping the model on the right side so that it is not blocked by texts. Maintain Title style of keeping the first letter capital
      { en_t:'New, Every Single Day.', en_s:'Latest Pakistani arrivals are added daily.', bn_t:'প্রতিদিন নতুন। সর্বশেষ কালেকশন।', bn_s:'প্রতিদিন নতুন পাকিস্তানি পণ্য যোগ হয়।' },
      // Change the background, keeping the model on the right side so that it is not blocked by texts. Maintain Title style of keeping the first letter capital
      { en_t:'Most competitive prices in BD.', en_s:'Confirmed PKR price, full BDT total, no hidden charges.', bn_t:'বিডিতে সবচেয়ে সাশ্রয়ী দাম।', bn_s:'নিশ্চিত পাকিস্তানি দাম, পূর্ণ বিডিটি টোটাল, কোনো লুকানো চার্জ নেই।' },
      // Change the background, keeping the model on the right side so that it is not blocked by texts. Maintain Title style of keeping the first letter capital
      { en_t:'Everyday to niche.', en_s:'From daily wear to the labels you love.', bn_t:'প্রতিদিন থেকে নিশ। প্রতিটি পাকিস্তানি ব্র্যান্ড।', bn_s:'প্রতিদিনের পোশাক থেকে প্রিয় ব্র্যান্ড।' },
      // Change the background, keeping the model on the right side so that it is not blocked by texts. Maintain Title style of keeping the first letter capital
    ...
    'home-k': [
      ...
      // Change the background, keeping the model on the right side so that it is not blocked by texts. Maintain Title style of keeping the first letter capital. Pakistani classics in little sizes should be on the second title line.
      { en_t:'Boys kurta, shalwar. Pakistani classics in little sizes.', en_s:'Traditional looks your little man will love.', bn_t:'বয়েজ কুর্তা সালোয়ার। পাকিস্তানি ক্লাসিক, ছোট সাইজে।', bn_s:'ঐতিহ্যবাহী লুক যা আপনার ছোট্ট মানুষটি ভালোবাসবে।' },
      // Change the background, keeping the model on the right side so that it is not blocked by texts. Maintain Title style of keeping the first letter capital
    ...
    'luxe-all': [
      // Change the background, keeping the model on the right side so that it is not blocked by texts. Maintain Title style of keeping the first letter capital.
      { en_t:'Door to Door. Authentic Products.', en_s:'Straight from Pakistan to your door in Bangladesh.', bn_t:'ডোর টু ডোর। আসল পণ্য।', bn_s:'পাকিস্তান থেকে সরাসরি বাংলাদেশে আপনার দরজায়।' },
      // Change the background, keeping the model on the right side so that it is not blocked by texts. Maintain Title style of keeping the first letter capital
      ...
      // Change the background, keeping the model on the right side so that it is not blocked by texts. Maintain Title style of keeping the first letter capital
      { en_t:'Chikankari. Zardozi. Real craft.', en_s:'Pakistani heritage, at the authentic PKR price.', bn_t:'চিকনকারি। জারদোজি। আসল কারুকাজ।', bn_s:'পাকিস্তানি ঐতিহ্যবাহী শিল্প, আসল PKR দামে।' },
      // Change the background, keeping the model on the right side so that it is not blocked by texts. Maintain Title style of keeping the first letter capital
    ...
    'luxe-w': [
      // Change the background, keeping the model on the right side so that it is not blocked by texts. Maintain Title style of keeping the first letter capital.For the big day should be on the second title line.
      ...
      // Change the background, keeping the model on the right side so that it is not blocked by texts. Maintain Title style of keeping the first letter capital
      { en_t:'Embroidered formals. Statement dressing.', en_s:'Couture-level craft, at honest PKR prices.', bn_t:'এমব্রয়ডারড ফরমাল। স্টেটমেন্ট ড্রেসিং।', bn_s:'কুতুর-মানের কারুকাজ, সৎ PK দামে।' }
    ],
    'luxe-m': [
      // Change the background, keeping the model on the right side so that it is not blocked by texts. Maintain Title style of keeping the first letter capital
      ...
      // Change the background image, the model of the image should be on the right side, so that it is not covered by texts. The title second line should be "Every occasion sorted." and first line should be Formal to festive.
    ...
    'luxe-k': [
      // Change the background, keeping the model on the right side so that it is not blocked by texts. Maintain Title style of keeping the first letter capital. Dresses to impress on the secornd title line
  ...
  function psCard(p, idx){
    ...
    const dualTag = (p.dual && p.altform)
      ...
      : '';
    // Fit-search results carry the buyer's matched size per product (fitSzLabel + fitConf). When
    // present, the available sizes render as CHIPS — the buyer's size filled gold, every other
    // in-stock size dimmed — so on each card it is unmistakable which of several sizes is theirs.
    const _fitLbl = p.fitSzLabel || '';
    const _norm = (s) => String(s).toLowerCase().replace(/[\s\/]/g, '');
    const _hits = _fitLbl ? [_norm(p.fitSize || ''), _norm(_fitLbl)] : [];
    let _anyHit = false;
    const _fitChips = _fitLbl
      ? list.slice(0, 8).map((s) => { const h = _hits.indexOf(_norm(s)) >= 0; if (h) _anyHit = true; return `<i class="${h ? 'ps-szc-hit' : 'ps-szc-dim'}">${esc(s)}</i>`; }).join('')
      : '';
    // guard: server only returns in-stock sizes, but if the matched label isn't among them, show it anyway
    const _fitChipsAll = _fitLbl ? ((_anyHit ? '' : `<i class="ps-szc-hit">${esc(_fitLbl)}</i>`) + _fitChips) : '';
    const _fitPre = (p.fitConf === 'high') ? tr('fit_b_your') : (p.fitConf === 'low') ? tr('fit_b_closest') : tr('fit_b_likely');
    const szList = list.slice(0, 7).map(esc).join(' · ');
    const szOverlay = isMto
      ...
      : isUns
        ...
        : _fitLbl
          ? `<div class="ps-img-sizes ps-img-sizes-fit">${dualTag}<b class="ps-sz-your">${_fitPre}</b><span class="ps-szchips">${_fitChipsAll}</span></div>`
          : `<div class="ps-img-sizes">${dualTag}<b>${tr('ps_avail_sizes')}</b>${szList}</div>`;
  ...
  // Share THIS product back into PakPoshak (not the brand's raw link): the recipient's tap
  // opens our own site straight to this popup, still inside our ecosystem. The product's own
  // fields ride along in the URL itself (no server lookup needed) — see psOpenSharedProduct.
  function psShareProduct(idx){
    const p = psFiltered[idx]; if(!p) return;
    const url = new URL(location.origin + location.pathname);
    url.searchParams.set('product', JSON.stringify({ u:p.u, t:p.t, b:p.b, img:p.img, pkr:p.pkr, cat:p.cat, sz:p.sz }));
    const text = p.b + ' — ' + p.t;
    if(navigator.share){
      navigator.share({ title: p.t, text: text, url: url.toString() }).catch(function(){});
      return;
    }
  ...
  // front of psFiltered so the existing psDetail(idx)/psAdd/psWishToggle machinery just works.
  function psOpenSharedProduct(raw){
    let p; try{ p = JSON.parse(raw); }catch(e){ return; }
    if(!p || !p.u || !p.t) return;
    psOnReady(function(){
      try{
        const key = psUrlKey(p.u);
        let i = psFiltered.findIndex(function(x){ return psUrlKey(x.u) === key; });
        if(i < 0){ psFiltered.unshift(p); i = 0; }
        psDetail(i);
        const clean = new URL(location.href); clean.searchParams.delete('product'); history.replaceState(null, '', clean.toString());
      }catch(e){}
    });
  }
  ...
  function psDetail(idx){
    ...
    document.getElementById('psDetailInner').innerHTML =
      ...
      +   `<div class="ps-d-pricerow">`
      +     `<div><div class="ps-d-price">≈ ৳${bdt.toLocaleString()}</div><div class="ps-d-pkr">PKR ${p.pkr.toLocaleString()}</div></div>`
      +     `<div class="ps-d-iconrow">`
      +       `<div id="psDSizeChart" class="ps-d-szchart-wrap"></div>`
      +       `<button type="button" class="ps-d-sharebtn" onclick="psShareProduct(${idx})" aria-label="${tr('ps_share')}" title="${tr('ps_share')}"><svg class="ps-share-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 12v6a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-6"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg></button>`
  ...
  function psPaintDetail(p){
    ...
    if(galEl && st.imgs.length){
      galEl.innerHTML = st.imgs.slice(0, 12).map((src, i) => `<img class="ps-d-shot" loading="${i < 2 ? 'eager' : 'lazy'}" src="${esc(src)}" alt="${esc(p.t)}">`).join('');
      if(st.imgs.length > 1) psDetailSwipeHintMaybe(galEl);
    }
    if(detEl){
      ...
      // short attributes (Colour/Fabric/Pieces/Fit) stay in the compact key/value table; long or
      // multi-line sections (Care/Disclaimer/Delivery/tab prose) render as full-width labelled blocks
      // so they read like the Description instead of cramping the 2-column table.
      const isLong = v => (v || '').length > 140 || /\n/.test(v || '');
      const shortRows = rows.filter(r => !isLong(r[1])), longRows = rows.filter(r => isLong(r[1]));
      let html = '';
      if(shortRows.length) html += `<div class="ps-d-details-h">${esc(tr('ps_d_details_h'))}</div><table class="ps-d-tbl"><tbody>`
        + shortRows.map(r => `<tr><td class="ps-d-k">${esc(r[0])}</td><td class="ps-d-v">${esc(r[1])}</td></tr>`).join('')
        + `</tbody></table>`;
      for(const r of longRows) html += `<div class="ps-d-sec"><div class="ps-d-sec-h">${esc(r[0])}</div><div class="ps-d-sec-b">${esc(cleanDesc(r[1]))}</div></div>`;
  ...
  // a popup gallery is only seen a handful of times, unlike the grid).
  function psDetailSwipeHintMaybe(el){
    try{
      if(!(navigator.maxTouchPoints > 0 || 'ontouchstart' in window)) return;
      if(localStorage.getItem('psb_popup_swipe_seen')) return;
      localStorage.setItem('psb_popup_swipe_seen', '1');
      el.classList.remove('ps-swipenudge'); void el.offsetWidth; el.classList.add('ps-swipenudge');
      setTimeout(function(){ if(el) el.classList.remove('ps-swipenudge'); }, 1700);
    }catch(e){}
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    try { var _pq = new URLSearchParams(location.search).get('product'); if(_pq) psOpenSharedProduct(_pq); } catch(e){}   // deep-link ?product=<data> (shared product)
